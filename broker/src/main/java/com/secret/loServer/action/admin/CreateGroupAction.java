...
public class CreateGroupAction extends AdminAction {
    ...
    public boolean action(Request request, Response response) {
        if (request.getNettyRequest() instanceof FullHttpRequest) {
            ...
            Map map = new HashMap();
            map.put("group", inputCreateGroup.getGroup());
            map.put("toLine", inputCreateGroup.getTo_lines());
            map.put("notifyContent", inputCreateGroup.getNotify_message());
            String json = JSONObject.toJSON(map).toString();
            System.out.println(json);
            //  if (inputCreateGroup.isValide()) {
            RPCCenter.getInstance().sendRequest(inputCreateGroup.getOperator(), null, IMTopic.CreateGroupTopic, json.getBytes(), inputCreateGroup.getOperator(), TargetEntry.Type.TARGET_TYPE_USER, new RPCCenter.Callback() {
                @Override
