...
import com.liuyan.im.MessageBundle;
...
public class DatabaseStore {
    ...
    void persistMessage(final WFCMessage.Message message, boolean update) {
        ...
        mScheduler.execute(() -> {
            ...
            try {
                ...
                System.out.println("插入消息sql：" + sql);
    ...
    void persistSensitiveMessage(final WFCMessage.Message message) {
        ...
        mScheduler.execute(() -> {
            ...
            try {
                ...
                statement = connection.prepareStatement(sql);
                System.out.println("插入消息sql2：" + sql);
                LOG.info("MS插入消息sql：{}" + sql);
                int index = 1;
                statement.setLong(index++, message.getMessageId());
                statement.setString(index++, message.getFromUser());
                statement.setInt(index++, message.getConversation().getType());
    ...
    boolean loadRemoteMessagesFromTable(String user, WFCMessage.Conversation conversation, long[] before, int count, String table, List<WFCMessage.Message> messages) {
        ...
        try {
            ...
            System.out.println("拉取消息sql：" + sql);
            System.out.println("conversation.getType():" + conversation.getType() + " conversation.getLine()" + conversation.getLine() + " beforeUid" + beforeUid + " " +
                    "conversation.getTarget()" + conversation.getTarget() + " count" + count);
    ...
    void persistUserMessage(final String userId, final long messageId, final long messageSeq) {
        mScheduler.execute(() -> {
            ...
            try {
                ...
                System.out.println("增加SQL message:" + sql);
                LOG.info("增加SQL message:{}" + sql);
                statement = connection.prepareStatement(sql);
                int index = 1;
    ...
    WFCMessage.User getPersistUser(String userId) {
        ...
        try {
            ...
            statement = connection.prepareStatement(sql);
            System.out.println("======查询用户sql" + sql);
            int index = 1;
            statement.setString(1, userId);
    ...
    public void persistOrUpdateFriendDataList(List<FriendData> insertDataList) {
    }
    ...
    public void persistUserToken(String clientId, String userId, String token) {
    }


    public Map<String, String> selectUserTokenByUserId(String userId) {
        return null;
    }

    public Map<String, String> selectUserTokenByClientId(String clientId) {
        return null;
    }


    public void updateUserTokenByUserId(String clientId, String userId, String token) {
    }

    public void updateUserTokenByClientId(String clientId, String userId, String token) {
    }

    public void updateSessionActive(MemorySessionStore.Session session) {
    }
}
