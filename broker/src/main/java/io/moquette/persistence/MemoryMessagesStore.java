...
import static com.liuyan.im.MyInfoType.*;
...
import static com.liuyan.im.UserSettingScope.kUserSettingPCOnline;
...
public class MemoryMessagesStore implements IMessagesStore {
    ...
    /**
     * 好友申请列表  key是用户id(userCode) value是申请列表集合
     */
    static final String USER_FRIENDS_REQUEST = "user_friends_request";

    /**
     * 用户详情列表  key是用户id  value是该用户详情
     */
    ...
    static final String USER_SEQ_MID = "user_seq_mid";
    static final String USER_CHATROOM_SEQ_MID = "user_chatroom_seq_mid";
    ...
    //存储user,msgHead 信息，用于接收广播消息后，正确发送msgHead
    private Map<String, Long> userMsgHead = new HashMap<>();
    ...
    private long mFriendRequestDuration = 0;

    //private long mFriendRejectDuration = 30 * 24 * 60 * 60 * 1000;
    private long mFriendRejectDuration = 0;
    ...
    @Override
    public List<String> getCommonFriendIdList(String userId) {
        return null;
    }

    @Override
    public List<String> getUserNotAddFriendList(String userId) {
        return null;
    }

    @Override
    public ErrorCode addFriendSoon(String fromUser, List<WFCMessage.UserRequest> requestList, Long addUserTimeMillis) {
...
//            //user 用户id
//            maps = userMessages.get(user);
            maps = (TreeMap<Long, Long>) m_Server.getHazelcastInstance().getMap(USER_SEQ_MID).get(user);
        } else {
...
//            TreeMap<Long, Long> maps = userMessages.get(user);
            IMap<String, TreeMap<Long, Long>> iMap = m_Server.getHazelcastInstance().getMap(USER_SEQ_MID);
            TreeMap<Long, Long> maps = iMap.get(user);
            if (maps == null) {
...
//            TreeMap<Long, Long> maps = chatroomMessages.get(target);
            IMap<String, TreeMap<Long, Long>> iMap = m_Server.getServer().getHazelcastInstance().getMap(USER_CHATROOM_SEQ_MID);
            TreeMap<Long, Long> maps = iMap.get(target);
            if (maps == null) {
                maps = new TreeMap<>();
//                chatroomMessages.put(target, maps);
    ...
    public ErrorCode addGroupMembers(String operator, boolean isAdmin, String groupId, List<WFCMessage.GroupMember> memberList) {
        ...
        //  System.out.println("群成员数：" + memberCount);
        //超过上限
        if (memberCount > 501) {
            System.out.println("超过群上限 ：groupId:" + groupId);
            LOG.error("超过群上限 ：groupId:" + groupId);
            return ErrorCode.ERROR_CODE_OVER_LIMIT;
        }
        if (!isAdmin) {
...
//            userMessages.remove(userId);
            m_Server.getHazelcastInstance().getMap(USER_SEQ_MID).remove(userId);
        } finally {
            mWriteLock.unlock();
        }
...
//            }
     ...
     * @param version 时间戳  查询这个时间戳之后添加的好友
     * @return
     */
    @Override
    ...
    public ErrorCode saveAddFriendRequest(String userId, WFCMessage.AddFriendRequest request, long[] head) {
        ...
        if (existRequest != null && existRequest.getStatus() != ProtoConstants.FriendRequestStatus.RequestStatus_Accepted) {
            ...
            if (mFriendRequestDuration > 0 && System.currentTimeMillis() - existRequest.getUpdateDt() > 0) {
                //下面代码是控制好友申请被拒绝后再次申请的时间   现在逻辑  被拒绝后 可以马上再次申请
        ...
        for (FriendData fd : friendDatas) {
            //删除好友后依然允许重新添加 fd.getState() == 0
            if (fd.getFriendUid().equals(request.getTargetUid()) && fd.getState() == 0) {
                friendData1 = fd;
                break;
            }
        }
    ...
    @Override
    public ErrorCode handlePublishFriendRequest(String userId, WFCMessage.HandleFriendRequest request, WFCMessage.Message.Builder msgBuilder, long[] heads) {
        HazelcastInstance hzInstance = m_Server.getHazelcastInstance();
        ...
        if (existRequest != null) {
            if (mFriendRequestExpiration > 0 && System.currentTimeMillis() - existRequest.getUpdateDt() > mFriendRequestExpiration) {
                ...
                if (request.getStatus() == ProtoConstants.FriendRequestStatus.RequestStatus_Accepted) {
                    MultiMap<String, FriendData> friendsMap = hzInstance.getMultiMap(USER_FRIENDS);
                    Collection<FriendData> friends = friendsMap.get(userId);
                    final String reason = existRequest.getReason();
                    if (CollectionUtil.isNotEmpty(friends)) {
                        friends.stream().filter(f -> request.getTargetUid().equals(f.getFriendUid())).findAny().ifPresent(f -> {
                            heads[0] = f.getTimestamp();
                            heads[1] = f.getTimestamp();

                            msgBuilder.setConversation(WFCMessage.Conversation.newBuilder().setTarget(userId).setLine(0).setType(ProtoConstants.ConversationType.ConversationType_Private).build());
                            msgBuilder.setContent(WFCMessage.MessageContent.newBuilder().setType(1).setSearchableContent(reason).build());
                        });
                    }
                }
                return ErrorCode.ERROR_CODE_SUCCESS;
    ...
    /**
     * 插入好友关系时 将产生两条记录  这个方法 将两个记录的state 从0改成1
     *
     * @param userId
     * @param friendUid
     * @param head
    ...
    public ErrorCode deleteFriend(String userId, String friendUid, long[] head) {
        ...
        for (FriendData data : user2Friends) {
    ...
    @Override
    public ErrorCode deleteFriendByOneWay(String userId, String friendUid, long[] head) {
        return ErrorCode.ERROR_CODE_SUCCESS;
    }

    @Override
    public void insertOrUpdateUserToken(String clientId, String userId, String token) {
    }

    @Override
    public void insertOrUpdateUserTokenByClientId(String clientId, String userId, String token) {
    }

    @Override
    public Map<String, String> findUserTokenByUserId(String userName) {
        return null;
    }
...
//            maps = userMessages.get(user);
    ...
    @Override
    public long getAndIncreaseMessageHead(String user) {
        Long msgHead = userMsgHead.get(user);
        if (msgHead == null) {
            msgHead = getMessageHead(user);
        }
        mWriteLock.lock();
        try {
            userMsgHead.put(user, ++msgHead);
            return msgHead;
        } finally {
            mWriteLock.unlock();
        }
