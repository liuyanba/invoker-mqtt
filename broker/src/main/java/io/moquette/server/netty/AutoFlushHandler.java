...
public class AutoFlushHandler extends ChannelDuplexHandler {
    ...
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        String nodeIdStr = Server.defaultConfig().getProperty(BrokerConstants.NODE_ID);
        LOG.info(NettyUtils.clientID(ctx.channel()) + "================================建立连接========时间:" + DateUtil.now() + "==========节点:" + nodeIdStr);
    ...
    public void handlerRemoved(ChannelHandlerContext ctx) throws Exception {
        LOG.info(NettyUtils.clientID(ctx.channel()) + "======================================断开连接========时间:" + DateUtil.now());
        destroy();
    }

    @Override
    public void channelRegistered(ChannelHandlerContext ctx) throws Exception {
        LOG.info(NettyUtils.clientID(ctx.channel()) + "======================================注册========时间:" + DateUtil.now());
    ...
    protected void channelIdle(ChannelHandlerContext ctx/* , IdleStateEvent evt */) throws Exception {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("刷新空闲的Netty通道 MqttClientId = {}.", NettyUtils.clientID(ctx.channel()));
        }
        ctx.channel().flush();
    }
